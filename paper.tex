...
We further show that SVD-compressed models quantize better: SVD followed by INT8 quantization yields a 5.3\% composite improvement over INT8 alone on Qwen2.5-1.5B, and the knowledge protection benefit of CF90 survives through INT8 quantization (72\% retention vs.\ 58\% unprotected).
However, at 0.5B parameters, CF90 substantially degrades generation fluency, increasing trigram repetition from 5\% to 34\%.
This degradation does not appear at 7B or above, where CF90 maintains 95\% instruction-following accuracy and 98\% code generation accuracy.
...
We also document an important limitation.
At small model scales (0.5B parameters), CF90 preserves factual knowledge but degrades generation fluency.
Models produce correct factual completions but exhibit high repetition rates on open-ended prompts.
This problem does not appear at 7B parameters or above, where instruction-following and code generation benchmarks remain intact.
We characterize this scale-dependent tradeoff and provide practical guidance for when CF90 should and should not be used.
...
CALDERA \citep{saha2024caldera} extends this to jointly quantize both the low-rank factors and the residual.
...
We evaluated CF90 on Qwen2.5-7B and Qwen2.5-32B to assess whether the generation quality degradation observed at 0.5B persists at larger scales.

\begin{table}[h]
\centering
\caption{CF90 at larger scales.}
\label{tab:scale}
\begin{tabular}{llcc}
\toprule
...
At 7B, CF90 maintains average benchmark performance with no degradation.
At 32B, instruction-following (95\%) and code generation (98\%) scores indicate that generation quality is fully preserved.
The repetition problem at 0.5B is a consequence of insufficient model capacity: freezing 90\% of a 24-layer model leaves only 2 layers for generation, which is not enough at small scale but is proportionally adequate at larger scales.
...
\paragraph{The generation quality tradeoff.}
CF90 at 0.5B presents a clear tradeoff: factual knowledge is preserved, but open-ended generation quality is lost.
The failure modes (empty outputs and repetition loops) are characteristic of models with very few effective parameters for generation.
With 22 of 24 layers frozen, only two transformer blocks and the language modeling head are free to adapt.
At 0.5B, these two layers have roughly 40M parameters, which is apparently below the threshold for coherent multi-token generation.
At 7B with the same 90\% freeze ratio, the top layers have approximately 700M free parameters, which is sufficient.

This suggests a practical rule: CF90 should be used on models where the unfrozen parameter count exceeds approximately 100--200M.
For a 90\% freeze, this means the full model should be at least 1--2B parameters.
...
Third, all multi-seed experiments were conducted on a single model family (Qwen2.5).
Results may differ on architectures with different attention patterns or layer counts.
...
Fifth, our INT8 quantization implementation (scale-and-round) is simpler than production methods like GPTQ \citep{frantar2022gptq} or AWQ \citep{lin2023awq}.
...
For models at 7B or above, apply CF90 before any fine-tuning that might conflict with pretrained knowledge.
For quantization pipelines, apply SVD compression before quantizing to improve the quality of the quantized model.
For models below 1B, use CF90 only when factual retention is more important than generation fluency.

The method is most relevant to practitioners running local models on constrained hardware, where both compression and knowledge preservation are critical.
As local deployment of language models continues to grow, methods that protect model quality through the compression pipeline will become increasingly important.
